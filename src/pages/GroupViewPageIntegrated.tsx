...
const GroupViewPage: React.FC = () => {
  ...
  const liveDetections = appCameraIds.map(appId => {
    const jsonCameraId = appCameraIdToJsonId[appId];
    if (jsonCameraId && currentFrameData?.cameras?.[jsonCameraId]?.tracks) {
      return currentFrameData.cameras[jsonCameraId].tracks.length;
    }
    return 0;
  });
  const maxDetections = Math.max(1, ...liveDetections);
                  ...
                  <div key={name} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center">
                      <div className="w-16 h-2 bg-gray-700 rounded-full mr-2">
                        <div className="h-2 bg-green-500 rounded-full" style={{ width: `${(liveDetections[idx] / maxDetections) * 100}%` }}></div>
                      </div>
                      <span className="font-mono">{liveDetections[idx]}</span>
                    </div>
                  </div>
                ))}
              </div>
